    ...
    class DeleteCommand : Command
    {
        public DeleteCommand(CommandLineApplication app, ILogger logger)
        {
            app.Command("delete", delete =>
            {
                ...
                //TODO: discuss with nuget team that 'quiet' is more widely used
                ...
                delete.OnExecute(async () =>
                {
                    ...
                    if (!Confirm(string.Format(message, packageId, packageVersion, pushCommandResource.GetPushEndpoint())))
                    {
                        await packageUpdater.DeletePackage(apiKey.Value(),
                           packageId,
                           packageVersion,
                           GetUserAgent(),
                           logger,
