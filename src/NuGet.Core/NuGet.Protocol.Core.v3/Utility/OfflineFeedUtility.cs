...
namespace NuGet.Protocol.Core.Types
{
    public static class OfflineFeedUtility
    {
            ...
            string nameOfNotFoundErrorResource)
        {
            ...
            {
                throw new ArgumentException(string.Format(CultureInfo.CurrentCulture,
                    Strings.ResourceManager.GetString(nameOfNotFoundErrorResource),
                    path));
            }
        }
            ...
            using (var packageStream = File.OpenRead(packagePath))
            {
                try
                {
                    ...
                    if (PackageExists(packageIdentity, source, out isValidPackage))
                    {
                        ...
                        if (isValidPackage)
                        {
                            var message = string.Format(
                                ...
                                Strings.AddCommand_PackageAlreadyExists,
                                packageIdentity, 
                                source);
                        ...
                        else
                        {
                            var message = string.Format(CultureInfo.CurrentCulture,
                                Strings.AddCommand_ExistingPackageInvalid, 
                                packageIdentity, 
                                source);
                    ...
                    else
                    {
                        ...
                        var message = string.Format(
                            CultureInfo.CurrentCulture,
                            Strings.AddCommand_SuccessfullyAdded,
                            packagePath,
                            source);
                ...
                catch (InvalidDataException)
                {
                    var message = string.Format(
                        CultureInfo.CurrentCulture,
                        Strings.NupkgPath_InvalidNupkg,
                        packagePath);
